...
import {
  ...
  removeTreatmentTag as removeTreatmentTagOnCard,
...
const LAST_PROTOCOL_KEY = 'aultfarms.treatments.lastProtocol';
...
function writeLastProtocol(protocol: string): void {
  try {
    window.localStorage.setItem(LAST_PROTOCOL_KEY, protocol);
  } catch {
    // Ignore storage failures; the in-session draft still keeps the last protocol.
  }
}

function mostRecentProtocol(records: LivestockRecords): string {
  const latest = records.treatments.records.slice()
    .sort((left, right) => right.dateLastActivity.localeCompare(left.dateLastActivity))[0];
  ...
  try {
    ...
    runInAction(() => {
      ...
      if (!state.draft.treatment) {
        state.draft.treatment = readLastProtocol() || mostRecentProtocol(loaded);
      }
...
export const saveTreatment = action('saveTreatment', async () => {
  ...
  try {
    ...
    runInAction(() => {
      applyLiveRecords(loaded);
      writeLastProtocol(state.draft.treatment);
...
async function reloadLiveRecords(): Promise<LivestockRecords> {
  const loaded = await livestockRecords.fetchRecords(await client());
  if (!loaded) throw new Error('Livestock board was not found');
  return loaded;
}

export const changeTreatmentProtocol = action(
  'changeTreatmentProtocol',
  async (record: TreatmentRecord, treatment: string) => {
    if (!state.records || state.saving) return;
    const next = treatment.trim();
    if (!next || next === record.treatment) return;
    state.saving = true;
    try {
      const result = await updateTreatmentRecord(await client(), state.records, {
        record,
        treatment: next,
      });
      const loaded = await reloadLiveRecords();
      runInAction(() => {
        applyLiveRecords(loaded);
        addActivity(result.status === 'unchanged'
          ? 'Treatment protocol was already set.'
          : 'Treatment protocol updated in Trello.');
      });
    } catch (error) {
      runInAction(() => {
        addActivity(`Treatment was not updated: ${error instanceof Error ? error.message : String(error)}`, 'bad');
      });
    } finally {
      runInAction(() => {
        state.saving = false;
...
export const removeTagFromTreatment = action(
  'removeTagFromTreatment',
  async (record: TreatmentRecord, tag: Tag) => {
    if (!state.records || state.saving) return;
    state.saving = true;
    try {
      const result = await removeTreatmentTagOnCard(await client(), state.records, { record, tag });
      const loaded = await reloadLiveRecords();
      runInAction(() => {
        applyLiveRecords(loaded);
        addActivity(result.status === 'closed'
          ? 'Treatment card closed after removing the last tag.'
          : 'Tag removed from the treatment card.');
      });
    } catch (error) {
      runInAction(() => {
